...
//	but ILogger level cannot change from outside. By default, ILogger uses the level of the named logger from where
//	it was created
type ILogger interface {
	...
	WithLevel(Level) ILogger //only affects this new logger (can use LevelDefault to reset to named level and allow external control)
	With(name string, value interface{}) ILogger
...
func (l logger) WithLevel(newLevel Level) ILogger {
	l.level = newLevel
	return l
}

func (l logger) With(name string, value interface{}) ILogger {
	d := l.data
