...
func test(t *testing.T, w *testWriter, l logger.ILogger) {
...
func TestCodeWriterFile(t *testing.T) {
	w := &testWriter{records: []logger.Record{}}
	cw := logger.NewCodeWriter(w, logger.LevelDebug)
	cw.SetFileLevel("github.com/stewelarend/logger_test/code-writer_test.go", logger.LevelInfo)
	l := logger.New().WithLevel(logger.LevelDebug)
...
func TestCodeWriterLine(t *testing.T) {
	w := &testWriter{records: []logger.Record{}}
	cw := logger.NewCodeWriter(w, logger.LevelDebug)
	cw.SetFileLineLevel("github.com/stewelarend/logger_test/code-writer_test.go", 17, logger.LevelInfo) //will not log debug
	l := logger.New().WithLevel(logger.LevelDebug)
	l.SetWriter(cw)
	test(t, w, l)
